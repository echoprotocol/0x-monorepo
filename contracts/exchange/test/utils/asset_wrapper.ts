    ...
    public async getBalanceAsync(userAddress: string, assetData: string): Promise<BigNumber> {
        ...
        switch (proxyId) {
            ...
            case AssetProxyId.ERC721: {
                ...
                const [assetProxyId, tokenAddress, tokenId] = await this._devUtils.decodeERC721AssetData.callAsync(assetData);
            ...
            case AssetProxyId.ERC1155: {
                ...
                // tslint:disable-next-line-no-unused-variable
                const [assetProxyAddress, tokenAddress, tokenIds] = await this._devUtils.decodeERC1155AssetData.callAsync(assetData);
            ...
            case AssetProxyId.MultiAsset: {
                // tslint:disable-next-line-no-unused-variable
                const [assetProxyId, amounts, nestedAssetData]  = await this._devUtils.decodeMultiAssetData.callAsync(assetData);
                const nestedBalances = await Promise.all(
                    nestedAssetData.map(async _nestedAssetData =>
                        this.getBalanceAsync(userAddress, _nestedAssetData),
                    ),
                );
    ...
    public async setBalanceAsync(userAddress: string, assetData: string, desiredBalance: BigNumber): Promise<void> {
        ...
        switch (proxyId) {
            ...
            case AssetProxyId.ERC1155: {
                ...
                const assetProxyWrapper = this._proxyIdToAssetWrappers[proxyId] as ERC1155ProxyWrapper;
                // tslint:disable-next-line-no-unused-variable
                const [assetProxyAddress, tokenAddress, tokenIds, tokenValues] = await this._devUtils.decodeERC1155AssetData.callAsync(assetData);
                const assetWrapper = assetProxyWrapper.getContractWrapper(tokenAddress);
    ...
    public async getProxyAllowanceAsync(userAddress: string, assetData: string): Promise<BigNumber> {
        ...
        switch (proxyId) {
            ...
            case AssetProxyId.ERC1155: {
                ...
                const assetProxyWrapper = this._proxyIdToAssetWrappers[proxyId] as ERC1155ProxyWrapper;
                // tslint:disable-next-line-no-unused-variable
                const [assetProxyAddress, tokenAddress] = await this._devUtils.decodeERC1155AssetData.callAsync(assetData);
    ...
    public async setProxyAllowanceAsync(
        ...
        switch (proxyId) {
            ...
            case AssetProxyId.ERC721: {
                ...
                if (!doesTokenExist) {
                    throw new Error(
                        `Cannot setProxyAllowance on non-existent token: ${tokenAddress} ${
                            tokenId
                        }`,
                    );
                }
